...
TEXTS = {
    "vi": {
        ...
        "app_title_main": "TOOL MES - A05/A06",
        ...
        "station_a05": "Trạm A05",
        "station_a06": "Trạm A06",
        ...
        "select_station_to_save": "Vui lòng chọn Trạm (A05 hoặc A06) trước khi lưu kết quả.",
        ...
        "log_filter_result_a05": "Kết quả lọc A05: {} mã.",
        "log_filter_result_a06": "Kết quả lọc A06: {} mã.",
...
class AutoPasteTool:
    ...
    def _show_main_app(self):
        if self.login_frame:
            self.login_frame.destroy()
        self.main_app = MainApp(self.root, self.username, self.password, self)
    ...
    def _show_log_history(self):
        ...
        try:
            ...
            if os.path.exists(LOG_FILE):
                ...
                log_text.insert(tk.END, "Chưa có lịch sử hoạt động.")
        except Exception as e:
            log_text.insert(tk.END, f"Lỗi khi đọc lịch sử: {e}")
        finally:
            hide_file(LOG_FILE)
    ...
    def _on_f8_pressed(self):
        if self.main_app:
            self.root.after(0, self.main_app._toggle_paste)

    def _on_f9_pressed(self):
        if self.main_app:
            self.root.after(0, self.main_app._stop_paste)

class MainApp:
    def __init__(self, root, username, password, parent):
        self.root = root
        self.username = username
        self.password = password
        self.parent = parent
        ...
        self.filtered_codes_a05 = []
        self.filtered_codes_a06 = []

        self.selected_station = tk.StringVar(value="A05")
    ...
    def _apply_language(self, lang=None):
        ...
        self.station_a05_radio.config(text=texts["station_a05"])
        self.station_a06_radio.config(text=texts["station_a06"])
        if self.is_pasting and not self.is_paused:
            self.start_paste_button.config(text=texts["pause_paste_button"])
        else:
    ...
    def _create_main_widgets(self):
        ...
        control_frame = ttk.LabelFrame(main_frame, text="Điều khiển Dán", padding="15")
        ...
        self.station_a05_radio = ttk.Radiobutton(station_radio_frame, text=texts["station_a05"], variable=self.selected_station, value="A05", command=self._update_progress_labels)
        self.station_a05_radio.grid(row=0, column=0, padx=5)
        self.station_a06_radio = ttk.Radiobutton(station_radio_frame, text=texts["station_a06"], variable=self.selected_station, value="A06", command=self._update_progress_labels)
        self.station_a06_radio.grid(row=0, column=1, padx=5)
        self.station_a05_radio.config(state=tk.DISABLED)
        self.station_a06_radio.config(state=tk.DISABLED)
        ...
        progress_frame = ttk.LabelFrame(main_frame, text="Tiến trình", padding="15")
        ...
        self.elapsed_time_value_label = ttk.Label(progress_frame, text="0 phút 0 giây")
    ...
    def _set_total_file_path(self, filepath):
        ...
        if hasattr(self, 'total_file_tooltip'):
            self.total_file_tooltip.text = filepath
    ...
    def _set_missing_file_path(self, filepath):
        ...
        if hasattr(self, 'missing_file_tooltip'):
            self.missing_file_tooltip.text = filepath
    ...
    def _update_paste_speed_label_text(self):
        ...
        if self.paste_speed_var.get() == "ultrafast":
            self.paste_speed_unit_label.config(text=texts["paste_speed_ultrafast_display"])
        else:
            self.paste_speed_unit_label.config(text=texts["paste_speed_unit_display"].format(speed=self.current_speed_value))
    ...
    def _start_filter_thread(self):
        ...
        threading.Thread(target=self._read_and_filter, daemon=True).start()
    ...
    def _read_and_filter(self):
        ...
        try:
            ...
            self._update_status_bar(texts["status_bar_loading"].format(os.path.basename(self._full_total_file_path)))
            df_total = pd.read_excel(self._full_total_file_path, engine='openpyxl')
            self._update_status_bar(texts["status_bar_loading"].format(os.path.basename(self._full_missing_file_path)))
            df_missing = pd.read_excel(self._full_missing_file_path, engine='openpyxl', header=None)
        except Exception as e:
            show_message("Lỗi", f"Lỗi đọc file: {e}", type="error")
            self.filter_button.config(state=tk.NORMAL)
            self._is_reading_file = False
            return

        self.total_df = df_total
        self.missing_df = df_missing
        self._is_reading_file = False
        self.root.after(0, self._perform_filter_logic)
    ...
    def _perform_filter_logic(self):
        ...
        station_a05_value = "A05"
        station_a06_value = "A06"
        ...
        try:
                    ...
                    if len(row) > upload_status_col_total_index:
                        ...
                        if (isinstance(status_value, bool) and status_value is True) or \
                            ...
                            if pd.notna(code):
                                ...
                                if cleaned_code:
                                    total_codes_true_set.add(cleaned_code.upper())
                except Exception:
                    pass

            missing_codes_a05_set = set()
            missing_codes_a06_set = set()
            for index, row in self.missing_df.iloc[1:].iterrows():
                try:
                    if len(row) > max(barcode_col_missing_index, station_col_missing_index):
                        ...
                        if code:
                            if station == station_a05_value:
                                missing_codes_a05_set.add(code)
                            elif station == station_a06_value:
                                missing_codes_a06_set.add(code)
                except Exception:
                    pass

            self.filtered_codes_a05 = list(total_codes_true_set - missing_codes_a05_set)
            self.filtered_codes_a06 = list(total_codes_true_set - missing_codes_a06_set)
            self.filtered_codes_a05.sort()
            self.filtered_codes_a06.sort()

            count_a05 = len(self.filtered_codes_a05)
            count_a06 = len(self.filtered_codes_a06)

            filter_summary = texts["filter_success"]
            if count_a05 > 0: filter_summary += "\n" + texts["codes_found_label"].format(count=count_a05, station="A05")
            else: filter_summary += "\n" + texts["no_codes_found"].format(station="A05")
            if count_a06 > 0: filter_summary += "\n" + texts["codes_found_label"].format(count=count_a06, station="A06")
            else: filter_summary += "\n" + texts["no_codes_found"].format(station="A06")

            show_message(texts["filter_button"], filter_summary)

            if count_a05 > 0 or count_a06 > 0:
            ...
            if selected_station == "A05":
                self.total_codes_to_paste = len(self.filtered_codes_a05)
            elif selected_station == "A06":
                self.total_codes_to_paste = len(self.filtered_codes_a06)
            else:
                self.total_codes_to_paste = 0
    ...
    def _reset_filtered_codes(self):
        self.filtered_codes_a05 = []
        self.filtered_codes_a06 = []
        if hasattr(self, 'view_filtered_button'):
            self.view_filtered_button.config(state=tk.DISABLED)
    ...
    def _update_station_radio_states(self):
        lang = self.parent.current_language.get()
        a05_has_codes = len(self.filtered_codes_a05) > 0
        a06_has_codes = len(self.filtered_codes_a06) > 0

        if a05_has_codes: self.station_a05_radio.config(state=tk.NORMAL)
        else: self.station_a05_radio.config(state=tk.DISABLED)
        if a06_has_codes: self.station_a06_radio.config(state=tk.NORMAL)
        else: self.station_a06_radio.config(state=tk.DISABLED)

        current_selection = self.selected_station.get()
        if not a05_has_codes and not a06_has_codes:
            self.start_paste_button.config(state=tk.DISABLED)
        elif current_selection == "A05" and a05_has_codes:
            self.start_paste_button.config(state=tk.NORMAL)
        elif current_selection == "A06" and a06_has_codes:
            self.start_paste_button.config(state=tk.NORMAL)
        else:
            self.start_paste_button.config(state=tk.DISABLED)

        if self.selected_station.get() == "A05":
            self.total_codes_to_paste = len(self.filtered_codes_a05)
        elif self.selected_station.get() == "A06":
            self.total_codes_to_paste = len(self.filtered_codes_a06)
        else:
            self.total_codes_to_paste = 0
    ...
    def _toggle_paste(self):
        ...
        texts = TEXTS[lang]
        selected_station = self.selected_station.get()
        codes_to_paste = self.filtered_codes_a05 if selected_station == "A05" else self.filtered_codes_a06

        if not codes_to_paste:
            if not self.filtered_codes_a05 and not self.filtered_codes_a06:
                show_message(texts["start_paste_button"], texts["please_filter_first"], type="warning")
            else:
                show_message(texts["start_paste_button"], texts["no_codes_found"].format(station=selected_station), type="warning")
            self._update_status_bar(texts["status_bar_ready"])
            return
        ...
        if self.is_pasting and not self.is_paused:
            ...
            if self.start_time is not None:
                self._total_elapsed_time_at_pause += (self.paused_time - self.start_time)
            self.start_time = None
    ...
    def _update_progress_labels(self):
        ...
        if self.is_pasting and not self.is_paused and self.start_time is not None:
            ...
            if self.current_speed_value is not None and self.current_speed_value >= 0:
                ...
                if codes_remaining > 0 and self.current_speed_value > 0:
                    ...
                    self.estimated_time_value_label.config(text="Hoàn thành!")
                else:
            ...
            self.elapsed_time_value_label.config(text="0 phút 0 giây")
    ...
    def _update_progress_labels_after_thread_exit(self, was_stopped):
        ...
        else: self.estimated_time_value_label.config(text="Hoàn thành!")
    ...
    def _show_filtered_codes(self):
        ...
        selected_station = self.selected_station.get()
        codes_to_show = self.filtered_codes_a05 if selected_station == "A05" else self.filtered_codes_a06
        ...
        for code in codes_to_show:
            code_list_text.insert(tk.END, str(code) + "\n")
    ...
    def _save_filtered_codes(self):
        ...
        selected_station = self.selected_station.get()
        codes_to_save = self.filtered_codes_a05 if selected_station == "A05" else self.filtered_codes_a06
